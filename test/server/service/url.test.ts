...
test('can get component URL', async () => {
  ...
  expect(data.components).toEqual([
    ...
    expect.objectContaining({
      url: 'http://localhost:4000/view/bbc-morph-bar',
    }),
    expect.objectContaining({
      url: 'http://localhost:4000/data/bbc-morph-baz',
    }),
  ]);
});
...
test('can override localhost in component URL', async () => {
  ...
  expect(data.components).toEqual([
    ...
    expect.objectContaining({
      url: 'http://foo.local.com:4000/view/bbc-morph-bar',
    }),
    expect.objectContaining({
      url: 'http://foo.local.com:4000/data/bbc-morph-baz',
    }),
  ]);
});
...
test('can override component type with config', async () => {
  ...
  expect(data.components).toEqual([
    expect.objectContaining({
      url: 'http://localhost:4000/data/bbc-morph-foo',
    }),
    expect.objectContaining({
      url: 'http://localhost:1234',
    }),
    expect.objectContaining({
      url: 'http://localhost:4000/view/bbc-morph-baz',
    }),
  ]);
});
...
test('component type override defaults', async () => {
  ...
  expect(data.components).toEqual([
    expect.objectContaining({
      name: 'bbc-morph-sport-media-asset-data',
      url: 'http://localhost:4000/data/bbc-morph-sport-media-asset-data',
    }),
  ]);
});
